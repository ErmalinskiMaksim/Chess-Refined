...
void ChessClient::connect(std::string_view path) {
    ...
    m_connection->connect(path);
}

void ChessClient::onConnect() {
    // requestCache();
}
...
void ChessClient::dispatchServerResponse(ServerResponse resp) {
    switch (resp.type) {
        ...
        case ReqType::PROMOTE: {
            ...
            auto piece = static_cast<PieceType>(resp.payload[0]);
        ...
        case ReqType::ASSIGN_COLOR: {
            ...
            m_cache.teamColor = static_cast<PieceColor>(resp.payload[0]);
            break;
        }
...
void ChessClient::requestPromotion(PieceType type) {
    std::println("[CLI]: requesting promotion...");
    m_connection->send(Serializer::serialize(m_cache.selectedPiece, type, ReqType::PROMOTE));
}
...
const ChessClient::BoardType& ChessClient::getBoardCache() const noexcept {
...
bool ChessClient::isMyTurn() const noexcept {
